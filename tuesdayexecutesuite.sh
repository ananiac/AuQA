...
if [ `ps -ef | grep pabot | wc -l` -lt 2 ];\
  then\
    echo "pabot is not running"\
    ##Used the below for Ubuntu machine
    cd /home/fc/automation/AuQA
    sudo pabot --pabotlib -d Reports/cleanReports --output cleanReports.xml --variable environment:config37  /home/fc/automation/AuQA/Testcases/cleanReports.robot >> Reports/executionLog.txt 2>&1 || echo $(date +%F_%H:%M:%S)- cleanReports failed >> Reports/executionLog.txt 2>&1
...
#pabot --pabotlib -d Reports/cleanReports --output cleanReports.xml --variable environment:config37  Testcases/cleanReports.robot >> Reports/executionLog.txt 2>&1  || echo $(date +%F_%H:%M:%S)- cleanReports failed >> Reports/executionLog.txt 2>&1
