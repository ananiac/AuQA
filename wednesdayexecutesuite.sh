...
if [ `ps -ef | grep pabot | wc -l` -lt 2 ];  then
    ...
    rebot  --name "GuardTest" --reporttitle "Test performed across different environment and different group" --outputdir Reports --output output.xml $rp_path/basichotGuard.xml $rp_path/deadSensorGuard.xml $rp_path/hotGuard.xml $rp_path/guardOrderMIX.xml 2>&1 | tee -a Reports/executionLog.txt
